...
public class XMLGenerator_NoWall : MonoBehaviour {
	...
	void CreateWall(){
		for (int i=0;i<100;i++)
			for (int j=0;j<100;j++){
				if (a[i][j]==1){
				if (i==0 || a[i-1][j]==0) CreateSingleWall(i,j,"Left");
				if (j==0 || a[i][j-1]==0) CreateSingleWall(i,j,"Down");
				if (i==99 || a[i+1][j]==0) CreateSingleWall(i,j,"Right");
				if (j==99 || a[i][j+1]==0) CreateSingleWall(i,j,"Up");
				}
			}
	}

	void CreateSingleWall(int x,int y,string direction){
		...
		Instantiate(wall,new Vector3(x*100+offsetx,50,y*100+offsety),rotation);
	}
